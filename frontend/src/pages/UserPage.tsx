...
import type { BackendUser, Follower, GitHubUser, LocationState, Repo, User } from "../features/user.types";
...
export default function UserPage(): React.ReactElement {
  ...
  const fetchFollowers = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const response = await axios.get<Follower[]>(`https://api.github.com/users/${user.login}/followers`);
  ...
  const handleFollowerClick = useCallback(async (followerLogin: string): Promise<void> => {
    ...
    try {
      ...
      try {
        const backendResponse = await axios.get(`http://localhost:5000/users/name/${followerLogin}`);
