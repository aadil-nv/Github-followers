...
export default function SearchBar() {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUsername(value);
    setLastSearchSource(null); // Reset search source indicator
  };

  // Handle search button click - fetch from database first, then GitHub API if needed
  const handleSearch = async () => {
    ...
    try {
      ...
      try {
        ...
        // User not in database, will fetch from GitHub
        if (
      ...
      // Check if user exists but has no public repos
      let reposData = [];
      try {
      ...
      try {
        ...
        // Continue even if save fails - user can still view the data
        setLastSearchSource('github');
      }
